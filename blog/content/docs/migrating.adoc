...
=== Migrating `exclude` and config overlays

Jekyll's `exclude:` array prevents files from being processed.
In Roq, the equivalent is `site.ignored-files` -- a comma-separated list of glob patterns.
Patterns are relative to the site directory and additive to Roq's defaults (files starting with `_`).

Jekyll uses `_`-prefixed collection directories (`_versions/`, `_guides/`).
After migration, collections move to `content/<name>/`, so strip the `_` prefix and append `/++**++` for recursive matching:

[source,yaml]
----
# Jekyll _only_latest_guides_config.yml
exclude:
  - _versions/2.*
  - _versions/3.*
----
...
[source,properties]
----
# config/application-only-latest-guides.properties
site.ignored-files=versions/2.*/**,versions/3.*/**
----

==== Config overlays → Quarkus profiles

Jekyll supports multiple `--config` files that merge at build time:
...
In Roq, each overlay config becomes a https://quarkus.io/guides/config-reference#profile-aware-files[profile-aware properties file].
The naming convention is `application-__profile__.properties`, placed alongside the main `application.properties`:
...
| `config/application-noguides.properties`

| `_only_latest_guides_config.yml`
| `config/application-only-latest-guides.properties`
|===

Activate a profile with the `QUARKUS_PROFILE` environment variable:
...
Properties inside profile-aware files do not need a `%profile.` prefix -- they apply automatically when the profile is active.

TIP: The `dev` profile is activated automatically by `mvn quarkus:dev`, so development-specific settings can go in `application-dev.properties` or as `%dev.` entries in the main `application.properties`.
