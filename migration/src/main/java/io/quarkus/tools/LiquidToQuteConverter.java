...
public class LiquidToQuteConverter {
    ...
    private String convertTwoArgFilters(String content) {
        ...
        // Where_not: | where_not: 'prop' -> list:whereNot namespace call
        // Generated by convertFindFirstPattern; uses namespace form since the base may not be a JsonArray
        Pattern whereNotPattern = Pattern.compile(
                "([\\w.]+(?:\\([^)]*\\))?)\\s*\\|\\s*where_not:\\s*'(\\w+)'\\s*\\|\\s*first");
        Matcher wnm = whereNotPattern.matcher(content);
        if (wnm.find()) {
            result = wnm.replaceAll("list:whereNot($1, '$2').first");
            conversionsApplied.add("Converted where_not filter");
            content = result;
        }
    ...
    private String convertFindFirstPattern(String content) {
        ...
        if (m.find()) {
            ...
            String replacement = "{% assign __firstMatch = " + list
                    + " | where_not: '" + filterProp + "' | first %}"
                    + lets;
